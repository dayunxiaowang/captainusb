# CaptainUSB constraints for the 75T-484 x1 Artix-7 board.
...
create_clock -name pcie_sys_clk_p -period 10.000 [get_ports pcie_clk_p]

set_clock_groups -asynchronous \
    -group [get_clocks net_clk] \
    -group [get_clocks net_ft601_clk] \
    -group [get_clocks -quiet {pcie_sys_clk_p txoutclk_x0y0 clk_125mhz_x0y0 clk_125mhz_mux_x0y0 clk_250mhz_x0y0 clk_250mhz_mux_x0y0 mmcm_fb userclk1}]
...
# Pack FT601 control signals into IOBs for minimal output delay.
set_property IOB TRUE [get_cells i_ft601_loopback/i_ft601_phy/ft601_oe_n_reg]
set_property IOB TRUE [get_cells i_ft601_loopback/i_ft601_phy/ft601_rd_n_reg]
set_property IOB TRUE [get_cells i_ft601_loopback/i_ft601_phy/ft601_wr_n_reg]

# The PCILeech-style PHY drives ft601_data from data_out[0] via a
# byte-swap assign. The swap is pure wire reordering (no LUT), so the
# output data registers CAN be packed into IOBs, eliminating ~1.4 ns
# of SLICE-to-pad routing and comfortably closing output timing.
# The tristate enable (oe) register controls the OBUFT.T for both
# DATA and BE; false-path it so the tool does not attempt to close
# timing on a path the FT601 never samples (oe only toggles during
# bus turnaround, never while WR_N=0).
set_property IOB TRUE [get_cells {i_ft601_loopback/i_ft601_phy/data_out_reg[0][*]}]
set_false_path -from [get_pins i_ft601_loopback/i_ft601_phy/oe_reg/C] -to [get_ports {{ft601_be[*]} {ft601_data[*]}}]

set_false_path -from [get_pins -quiet -hierarchical -filter {NAME =~ *user_reset_out_reg*/C}]
